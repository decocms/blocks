...
`@decocms/runtime` (the shared otel/observability SDK) plus `@decocms/tanstack` (the Cloudflare Workers wiring) ship a thin, opinionated observability layer for Deco storefronts on Cloudflare Workers. Three signals — spans, logs, and metrics — flow to the same downstream (stats-lake / ClickStack) along two complementary transport paths:
...
`commerce_request_duration_ms` owned by the framework (Phase 2 / D-11) so every site emits it as soon as `@decocms/runtime` is bumped, regardless of `@decocms/apps` version. Apps register operation strings via `recordCommerceMetric`; the framework owns the cardinality contract.
...
- `deco.runtime.version` — `@decocms/runtime` build-time constant
...
import { instrumentWorker } from "@decocms/runtime/sdk/otel";
...
```ts
import { injectTraceContext } from "@decocms/runtime/sdk/observability";
...
`createInstrumentedFetch(name)` from `@decocms/runtime/sdk/instrumentedFetch` produces spans named `${name}.fetch` by default. For commerce calls where operators want to query by SEMANTIC endpoint (`vtex.intelligent-search.product_search`, `vtex.checkout.orderForm`) rather than by URL, supply an `operation` per call or wire a URL-derived router on the integration:
