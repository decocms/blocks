...
- **Proven old→new import mapping** (evidenced from `casaevideo-tanstack`'s actual, verified migration commits `7133613`, `55e353d`, `9585417`, `9393bf9` — apply exactly, don't re-derive from scratch):
  ...
  | `apps` | Unconfirmed — likely `@decocms/blocks-admin/apps/autoconfig`, but not evidenced in any real migration commit (only referenced inside `@decocms/apps`'s own internals, never by a site directly). **Verify empirically in Task 1** by reading what `@decocms/start/apps` actually exported in a pre-split checkout, before relying on this mapping in later tasks. | |
