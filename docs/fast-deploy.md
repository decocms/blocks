...
`index:revision` **must** equal `computeRevision(blocks)` (`packages/blocks/src/cms/blockSource.ts`,
...
- `packages/blocks/src/cms/blockSource.ts` — `BlockSource` interface, `BundledBlockSource`,
