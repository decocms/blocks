...
| 2026-09-14 | **D7 — Deploy moves to the control-plane (supersedes D6.3)** | D6.3's promised "follow-up D-record once shipped" — Cloudflare Workers Builds is replaced by our own pipeline. [`decocms/control-plane`](https://github.com/decocms/control-plane)'s `SITE_CREATE` MCP tool provisions a site end to end (GitHub repo, admin `sites` row, org ownership, the site's **own** CF KV/R2/D1 namespaces, the production env, the Deco CR); the operator applies the CR; [`decocms/infra_applications`](https://github.com/decocms/infra_applications) `images/cfworkers-builder` builds and deploys. Three things CF Builds structurally could not give us: (1) **per-site KV isolation** — `index:live` is one key per namespace, so two sites sharing one clobber each other's live pointer; `ensureSiteKvNamespace` creates `deco-kv-<site>` and the builder re-forces the `DECO_KV` id from `CF_KV_NAMESPACE_ID` every build, so a tenant repo cannot bind another site's KV. (2) **Seed-before-activate** — the builder seeds `decofile:<sha>` before `wrangler deploy` and flips `index:live` after, so new code never reads old content; that ordering is precisely what makes it safe to stub `blocks.gen` out of the server bundle (`DECO_SEEDED_DEPLOY`), which is the fix for the decofile being resident three times per isolate. A site on CF Builds can never take that win. (3) No per-worker dashboard click, and the builder strips tenant-controlled `routes`/`account_id`. **Unchanged from D6.3:** storefronts still do NOT get `deploy.yml`/`preview.yml`/`sync-secrets.yml` scaffolded — the pipeline is the control-plane's, not the site repo's. Site repos still commit `wrangler.jsonc`, which must declare the `DECO_KV` binding + `DECO_FAST_DEPLOY`; the control-plane personalizes ids but never adds a binding the repo did not declare. Wired into the migration as `provisionSite` (`scripts/migrate.ts`), gated on `DECO_CONTROL_PLANE_TOKEN` + `DECO_ORG`, inert without them. |
