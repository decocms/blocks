...
// NEW (useSignalValue from ~/sdk/signal — generated by the migration scaffold)
import { useSignalValue } from "~/sdk/signal";
const { displayCart } = useUI();
const open = useSignalValue(displayCart);
```

Both are equivalent — `useSignalValue` uses `useSyncExternalStore` + `sig.subscribe()` internally. Prefer `useSignalValue` when the `~/sdk/signal` shim is already imported in the file; prefer `useStore` when working with the `.store` directly.

Write-only consumers (event handlers) don't need either:
```typescript
