...
- **routes/router/workerEntry** → `@decocms/tanstack` root: `cmsRouteConfig`, `cmsHomeRouteConfig`, `loadCmsPage`, `loadCmsHomePage`, `loadDeferredSection`, `decoInvokeRoute`, `decoMetaRoute`, `decoRenderRoute` (plus, since 7.9.1, the preferred `decoMetaRouteConfig`/`decoRenderRouteConfig`/`decoInvokeRouteConfig` factories), `withSiteGlobals`, `createDecoRouter`, `createDecoWorkerEntry`; `@decocms/start/vite` → `@decocms/tanstack/vite`
  - **Dev-HMR footgun — admin route literals**: while rewriting `src/routes/deco/{meta,render,invoke.$}.ts`, never emit `createFileRoute("/deco/meta")(decoMetaRoute)` — the literal is a shared module-scope object that router-core's `update()` mutates (injects `id`/`path`), so any dev-HMR re-execution throws `Route cannot have both an 'id' and a 'path' option` and 500s every route until restart. Use the factory: `createFileRoute("/deco/meta")(decoMetaRouteConfig())` (or spread the literal on pre-7.9.1: `({ ...decoMetaRoute })`).
