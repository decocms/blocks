...
Proven on three production TanStack storefronts (one of them supplied the canonical three-commit sequence). Target `^7.6.0` at minimum; `^7.7.0` removes two workarounds (noted below).
...
3. **Typecheck diff-vs-baseline, not absolute zero**: capture `tsc --noEmit` output on the pre-migration commit, again after each migration commit, and diff the error sets. Sites have pre-existing errors (one reference site's baseline was 258); the gate is **zero new errors**, not zero errors.
