...
5. **`loadDecofileDirectory` vs. a generated-blocks import** — only reach for `loadDecofileDirectory` if the site has an existing directory of legacy per-page block JSON files to load at runtime. If blocks are generated at build time into one file, import that file and pass it to `setBlocks` directly; no directory loader needed. **For Next.js sites, the recommended shape of that generated file is the static-import manifest**: `@decocms/blocks-cli`'s `generate-blocks-manifest.ts` emits `.deco/blocksManifest.gen.ts`, which statically imports every `.deco/blocks/*.json` (keys identical to `loadDecofileDirectory`'s, so it's a drop-in `setBlocks(blocks)` / `createNextSetup({ blocks, blocksDir: false })` replacement). Because the block JSONs then live in Next's module graph, content edits hot-reload in `next dev` and deploys don't need `outputFileTracingIncludes` for `.deco/blocks` — the runtime `loadDecofileDirectory` read gives you neither. Re-run the generator when block files are added/removed (wire into the site's `generate` chain); see `@decocms/nextjs`'s README ("Recommended block source") for the full trade-offs.
