# @decocms/runtime / @decocms/tanstack — Framework TODO
...
- **Now**: `createSiteSetup()` (`@decocms/runtime/setup`) + `createAdminSetup()` (`@decocms/admin/setup`) absorb most of this. Sites still need a `setup.ts` file that calls them, so the boilerplate isn't zero, but it's a single call each rather than hand-wiring the registry.
...
- **Ideal**: export `SiteTheme` from `@decocms/runtime` or `@decocms/apps` so sites stop diverging.
...
- **Ideal**: `@decocms/runtime`/`@decocms/tanstack` exports a `<DecoAnalytics gtmId="GTM-XXX" />` component for `__root.tsx`.
