...
function generateMeta(): MetaResponse {
  ...
  // ---------------------------------------------------------------------------
  // Ensure the canonical site app is always registered.
  //
  ...
  //
  // The third pass (src/apps/) handles sites whose site.ts exports a Props
  // interface. As a fallback for sites that don't, we emit a built-in schema
  // with the well-known CMS-editable fields (seo, github, platform). If the
  // third pass already registered the site app with real properties we keep
  // those and only add the built-in fields that are still missing.
  // ---------------------------------------------------------------------------
  const SITE_APP_KEY = `${SITE_NAMESPACE}/apps/site.ts`;
  const SITE_APP_DEF_KEY = "SiteApp";
  const SITE_APP_B64_KEY = toBase64(SITE_APP_KEY);

  const BUILTIN_SITE_APP_PROPS: Record<string, any> = {
    seo: {
      title: "SEO",
      type: "object",
      properties: {
    ...
    github: {
      ...
      type: "object",
      properties: {
        owner: { type: "string", title: "Organization / owner" },
        repo: { type: "string", title: "Repository" },
        branch: { type: "string", title: "Default branch" },
        team: { type: "string", title: "GitHub team slug" },
      },
    },
    platform: {
      title: "Active Commerce Platform",
      type: "string",
      enum: ["vtex", "vnda", "shopify", "wake", "linx", "nuvemshop", "custom"],
    },
  };

  if (!appBlocks[SITE_APP_KEY]) {
    // Third pass didn't find src/apps/site.ts or it had no Props — emit built-in.
    definitions[SITE_APP_DEF_KEY] = {
      title: "Site",
      type: "object",
      properties: { ...BUILTIN_SITE_APP_PROPS },
    };
    definitions[SITE_APP_B64_KEY] = { $ref: `#/definitions/${SITE_APP_DEF_KEY}` };
    appBlocks[SITE_APP_KEY] = { $ref: `#/definitions/${SITE_APP_DEF_KEY}`, namespace: SITE_NAMESPACE };
    appRootAnyOf.push({ $ref: `#/definitions/${SITE_APP_DEF_KEY}` });
    console.log(`  ✓ ${SITE_APP_KEY} (built-in schema)`);
  } else {
    // Third pass registered it — ensure the built-in fields exist and add b64 alias.
    const appRef = appBlocks[SITE_APP_KEY].$ref as string;
    const defKey = appRef.replace("#/definitions/", "");
    const existing = definitions[defKey];
    if (existing) {
      existing.properties = { ...BUILTIN_SITE_APP_PROPS, ...(existing.properties ?? {}) };
      definitions[SITE_APP_DEF_KEY] = existing;
      definitions[SITE_APP_B64_KEY] = { $ref: `#/definitions/${SITE_APP_DEF_KEY}` };
      appBlocks[SITE_APP_KEY] = { $ref: `#/definitions/${SITE_APP_DEF_KEY}`, namespace: SITE_NAMESPACE };
    }
  }
