...
1. **`src/server/invoke.gen.ts`** (TanStack RPC path). Generated by `bunx tsx node_modules/@decocms/cli/scripts/generate-invoke.ts`. Audit:
   - File exists?
...
2. **`packages/admin/src/admin/invoke.ts`** (`/deco/invoke/...` HTTP path, mounted into the site via `@decocms/admin`). The single + batch invoke handlers must use `Headers.getSetCookie()` (not `entries()`!) when copying `RequestContext.responseHeaders` onto the response, via `forwardCtxHeadersTo()`. None of the `@decocms/*` packages are published yet — everything sits at `0.0.0` and consuming sites link against a local checkout via `bun link` (see the repo's README "Local development" section). There's no version to pin; just confirm the linked checkout's `admin/invoke.ts` has the `getSetCookie()`-based fix.
