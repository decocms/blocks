...
`packages/nextjs` returns zero hits — this table described the old Fresh/Deno
...
| `server-timing` | `?__d` debug mode (still real — see `packages/blocks/src/middleware/decoState.ts`) | Loader timings and cache status for the page-level response |
