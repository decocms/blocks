...
The generator lives at `packages/cli/scripts/generate-invoke.ts` in this repo (published as `@decocms/cli`, consumed by sites at `node_modules/@decocms/cli/scripts/generate-invoke.ts`). It moved here from the old single-package `@decocms/start` when the framework split into `runtime`/`admin`/`cli`/`tanstack`/`next` (see root `README.md`). It is framework-aware — it knows about `createServerFn` and TanStack Start's compiler constraints.
...
| **Config** | `runtime.ts` (3 lines) | `@decocms/cli`'s `generate-invoke.ts` (build script) |
