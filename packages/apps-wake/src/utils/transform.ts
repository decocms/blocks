...
export const camposAdicionais = ["Atacado", "Estoque", "Atributo", "Informacao", "TabelaPreco"];
...
export const toFilterItem = (filter: WakeFilterItem, base: URL) => ({
  ...
  values:
    filter?.values?.map((filterValue) => {
      ...
      const index = filterParams.findIndex((f) => (f.split("__")[1] || f.split(":")[1]) === name);
...
export const toPriceFilter = (
  filter: NonNullable<
    NonNullable<NonNullable<SearchQuery["result"]>["aggregations"]>["priceRanges"]
  >,
  base: URL,
) => ({
  "@type": "FilterToggle" as const,
  ...
  values:
    filter.map((filterValue) => {
      ...
      const filterParams = isDirectParam ? isDirectParam : url.searchParams.getAll(FILTER_PARAM);

      const index = isDirectParam
        ? filterParams.indexOf(name)
        : filterParams.findIndex((f) => (f.split("__")[1] || f.split(":")[1]) === name);
      ...
      if (selected) {
        const params = new URLSearchParams();
        url.searchParams.forEach((value, key) => {
          if ((key !== FILTER_PARAM && key !== "precoPor") || !value.endsWith(name)) {
            params.append(key, value);
          }
        });
        url.search = `${params}`;
...
export const toProduct = (
  ...
  if ((variant as SingleProductFragment)?.attributeSelections) {
    (variant as SingleProductFragment)?.attributeSelections?.selections?.forEach((selection) => {
  ...
  if (variant.promotions) {
    variant.promotions.forEach((promotion) => {
      additionalProperty.push({
        "@type": "PropertyValue",
  ...
  const review =
    (variant as SingleProductFragment).reviews?.map((review) => ({
  ...
  const isSimilarTo = variant.similarProducts?.map((p) => toProduct(p!, { base }));
  ...
  return {
    ...
    sku: variant.sku!,
    description:
      variant.informations?.find((info) => info?.type === "Descrição")?.value ?? undefined,
