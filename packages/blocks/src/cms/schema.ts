...
export function composeMeta(siteMeta: MetaResponse, options?: ComposeMetaOptions): MetaResponse {
  ...
  // Since self-contained meta.gen.json is now baked at generation time (the CLI
  // always composes before writing), the tanstack runtime re-runs composeMeta
  // on the file it loads from disk. A composed meta always carries a `framework`
