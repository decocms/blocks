...
function toSection(resolveType: string, props: Record<string, unknown>): Section {
  return { __resolveType: resolveType, ...props } as Section;
}

/**
 * Converts a Spire Block array into a Section[] using blog/sections/blocks
 * components. Sections are sorted by block position before conversion.
 * Supports an optional override map to remap block types to custom section
 * renderers.
 */
export function blocksToSections(
  blocks: Block[],
  overrides: Record<string, { __resolveType?: string }> = {},
): Section[] {
  return [...blocks]
...
function blockToSection(
  ...
  if ("type" in block && block.type) {
    if (overrides[block.type]) {
      return toSection(overrides[block.type]?.__resolveType ?? block.type, content);
    }
