import type { Author, BlogPost, Category, Publisher } from "../types";
...
const toAuthor = (author: Author) => {
  ...
  return {
    ...
    ...(type === "Person" && author.jobTitle ? { jobTitle: author.jobTitle } : {}),
...
export const toOrganization = (publisher: Publisher) => ({
  ...
  ...(publisher.logo ? { logo: { "@type": "ImageObject" as const, url: publisher.logo } } : {}),
});
 ...
 */
export const toBlogPosting = (post: BlogPost, url?: string, publisher?: Publisher) => {
  const image = post.seo?.image || post.image;
  const categories = post.categories?.map((category) => category.name).filter(Boolean);
  ...
  const aggregateRating =
    post.aggregateRating?.ratingValue != null &&
    (post.aggregateRating.ratingCount != null || post.aggregateRating.reviewCount != null)
      ? post.aggregateRating
      : undefined;
  const interactionStatistic =
    post.interactionStatistic?.userInteractionCount != null ? post.interactionStatistic : undefined;
  ...
  return {
    ...
    ...(post.readTime && post.readTime > 0 && Number.isFinite(post.readTime)
      ...
      : {}),
    ...(url ? { url, mainEntityOfPage: { "@type": "WebPage" as const, "@id": url } } : {}),
...
export const toBreadcrumbList = (
  url: string,
  { currentName, categories }: { currentName?: string; categories?: Category[] } = {},
) => {
  const { origin, pathname } = new URL(url);
  const segments = pathname.split("/").filter(Boolean);

  const nameOf = (segment: string) =>
    categories?.find((category) => category.slug === segment)?.name ?? humanize(segment);
  ...
  const itemListElement = segments.map((segment, index) => {
    ...
    return {
      ...
      ...(isLast ? {} : { item: `${origin}/${segments.slice(0, index + 1).join("/")}` }),
    };
  });
