import { hardSanitize } from "../../utils/hardSanitize";
...
export default function Table({ headers, rows }: Props) {
  ...
  return (
    <div className="my-8 border border-line rounded-brand overflow-x-auto">
      <table className="w-full border-collapse text-left">
          ...
          <thead>
            <tr className="bg-alt">
              {head.map((cell, i) => (
                <th
                  key={i}
                  className="px-4 py-3 text-xs font-semibold tracking-caps uppercase border-b-2 border-line"
        ...
        <tbody>
          {body.map((row, r) => (
            <tr key={r} className="border-b border-line-subtle last:border-b-0">
              {row.map((cell, c) => (
                <td
                  key={c}
                  className={cellClass}
