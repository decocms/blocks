...
export default function CardGroup({ cards }: Props) {
  ...
  return (
    <div className={`grid gap-4 my-10 ${gridCols}`}>
      {clamped.map((card, i) => (
        <div
          key={i}
          className="flex flex-col gap-3 p-6 sm:p-8 bg-alt border border-line rounded-brand transition-shadow hover:shadow-brand"
        >
          {card.icon && (
            <div
              className="flex items-center justify-center w-9 h-9 text-lg bg-base border border-line rounded-brand-sm flex-shrink-0 leading-none"
              aria-hidden="true"
            >
              {card.icon}
            </div>
          ...
          />
          <p
            className="text-sm leading-normal [text-wrap:pretty] m-0"
