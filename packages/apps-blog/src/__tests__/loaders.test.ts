...
describe("BlogpostListing", () => {
  it("returns paginated listing with defaults (page=1, count=12, sortBy=date_desc)", async () => {
    const result = await BlogpostListing({});
  ...
  it("URL search params override props", async () => {
    const req = new Request("http://localhost/blog?page=2&count=1");
    const result = await BlogpostListing({}, req);
    expect(result).not.toBeNull();
  ...
  it("computes nextPage / previousPage correctly", async () => {
    const result = await BlogpostListing({ count: 1, page: 2 });
    expect(result).not.toBeNull();
    expect(result!.pageInfo.nextPage).toContain("page=3");
    expect(result!.pageInfo.previousPage).toContain("page=1");
  });

  it("returns null when no posts match", async () => {
    mockGetRecords.mockReturnValue([]);
    expect(await BlogpostListing({})).toBeNull();
  });

  it("SEO canonical strips query params", async () => {
...
describe("BlogRelatedPostsLoader", () => {
  it("excludes current post via excludePostSlug", async () => {
    const result = await BlogRelatedPostsLoader({ excludePostSlug: "post-a" });
    expect(result).not.toBeNull();
    expect(result!.find((p) => p.slug === "post-a")).toBeUndefined();
  });

  it("returns BlogPost[] (not BlogPostListingPage)", async () => {
    const result = await BlogRelatedPostsLoader({});
