...
describe("sortPosts", () => {
  ...
  it("sorts date_desc — most recent first", async () => {
    const sorted = await sortPosts(posts, "date_desc");
    expect(sorted.map((p) => p.slug)).toEqual(["c", "b", "a"]);
  });

  it("sorts date_asc — oldest first", async () => {
    const sorted = await sortPosts(posts, "date_asc");
    expect(sorted.map((p) => p.slug)).toEqual(["a", "b", "c"]);
  });

  it("sorts title_asc", async () => {
    const sorted = await sortPosts(posts, "title_asc");
    // localeCompare(a,b) is negated for asc → Z-A
    expect(sorted.map((p) => p.title)).toEqual(["Charlie", "Bravo", "Alpha"]);
  });

  it("sorts title_desc", async () => {
    const sorted = await sortPosts(posts, "title_desc");
    // localeCompare(a,b) kept as-is for desc → A-Z
    expect(sorted.map((p) => p.title)).toEqual(["Alpha", "Bravo", "Charlie"]);
  });

  it("falls back to date for invalid sort field", async () => {
    const sorted = await sortPosts(posts, "invalid_desc" as SortBy);
    expect(sorted.map((p) => p.slug)).toEqual(["c", "b", "a"]);
  });

  it("falls back to desc for invalid sort order", async () => {
    const sorted = await sortPosts(posts, "date_wrong" as SortBy);
    expect(sorted.map((p) => p.slug)).toEqual(["c", "b", "a"]);
  });

  it("sorts posts with missing date to end", async () => {
    const withMissing = [
      makePost({ slug: "no-date", date: undefined as unknown as string }),
      makePost({ slug: "has-date", date: "2024-06-01" }),
    ];
    const sorted = await sortPosts(withMissing, "date_desc");
    expect(sorted[0].slug).toBe("has-date");
    expect(sorted[1].slug).toBe("no-date");
  });

  it("throws on empty array (accesses blogPosts[0])", async () => {
    await expect(sortPosts([], "date_desc")).rejects.toThrow();
  });

  it("does not mutate original array", async () => {
    const original = [...posts];
    await sortPosts(posts, "date_asc");
...
describe("handlePosts", () => {
  ...
  it("slug string + postSlugs — uses slug filtering", async () => {
    const result = await handlePosts(posts, "date_desc", "news", ["a", "c"]);
    // sorted by date_desc: c (March) before a (Jan)
    expect(result?.map((p) => p.slug)).toEqual(["c", "a"]);
  });

  it("slug string + term — chains category + term filters", async () => {
    const result = await handlePosts(posts, "date_desc", "news", undefined, "Alpha");
    expect(result?.map((p) => p.slug)).toEqual(["a"]);
  });

  it("slug as string[] — triggers related posts path", async () => {
    const result = await handlePosts(posts, "date_desc", ["tech"]);
    expect(result?.map((p) => p.slug)).toEqual(["c", "b"]);
  });

  it("no slug + term — term-only filtering", async () => {
    const result = await handlePosts(posts, "date_asc", undefined, undefined, "Bravo");
    expect(result).toHaveLength(1);
    expect(result![0].slug).toBe("b");
  });

  it("excludePostSlug removes matching post", async () => {
    const result = await handlePosts(posts, "date_desc", undefined, undefined, undefined, "b");
    expect(result?.find((p) => p.slug === "b")).toBeUndefined();
  });

  it("returns null when result is empty", async () => {
    expect(await handlePosts(posts, "date_desc", "nonexistent")).toBeNull();
  });

  it("returns all posts sorted when no filters given", async () => {
    const result = await handlePosts(posts, "date_desc");
