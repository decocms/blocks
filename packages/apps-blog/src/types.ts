import type { PageInfo, Person, Thing } from "@decocms/apps-commerce/types";
import type { ImageWidget } from "@decocms/apps-website/types";
import { scheduledTime } from "./utils/date";

export type { PageInfo };

/**
 * Opaque "pick any section" reference, as authored in the decofile:
 * `{ "__resolveType": "site/sections/Foo.tsx", ...props }`.
 *
 * Deliberately `any`, and deliberately named `Section`. `@decocms/blocks-cli`'s
 * schema generator only emits the admin's section-picker widget for a prop whose
 * type is *named* `Section` **and** resolves to `any`/`unknown`
 * (`generate-schema.ts`, `isOpaqueSectionType`) — a concrete object shape is
 * treated as user data and rendered as an inline editable form instead. So this
 * is not laziness: narrowing it would silently downgrade the editor experience.
 *
 * Note this is NOT `Section` from `@decocms/blocks/types`, which is the
 * *resolved* `{ Component, props }` pair produced after block resolution.
 */
...
export const isLivePost = (
  ...
  const goLive = post.scheduledDatetime ? scheduledTime(post.scheduledDatetime) : null;
...
export interface Rating {
  ...
  worstRating?: number;
  /** A short explanation providing background context and other information that led to the conclusion expressed in the rating. */
  ratingValue?: number;
  /** Review status */
  additionalType?: string;
}
